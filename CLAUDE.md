...
An **environment** is a named bundle of Thing Models with **fixed instance ids**, optional per-Thing state overrides, optional URI→id aliases, and an optional `clock` to pin — one JSON manifest in `src/environments/<name>.json` (`src/things/environments.ts`). It is the reproducible unit a benchmark runs against: the same Things, ids and initial state, one command.
