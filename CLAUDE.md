...
**Nothing is instantiated implicitly.** `src/things/` is a *catalog of Thing Models*, not a roster of what runs. There is exactly one way a Thing comes into existence — `ThingRegistry.instantiate()` — and both entry points go through it: the `--things` flag at startup (`parseArgs` in `src/config/options.ts`) and the lab API the dashboard calls (`src/http/labApi.ts`). One path in means one identity scheme; there is no config file.
...
- Reference resolution is TD-informed: a bare identifier naming an action **input parameter** resolves to that input value; a bare identifier naming a **Thing property** resolves to `state[name]`; effect targets (LHS) must be Thing properties. An object `input` schema exposes its properties by name; a scalar `input` is referred to as `input`. Cross-Thing/dotted refs are not supported (single-Thing loader).

Examples in the tree: `src/things/counter/counter.td.json` (`"vre:effects": "count' = count + 1; emitEvent(\"change\", count);"`), `src/things/lamp/lamp.td.json` (`"on' = !on;"`, and `brightness' = on ? input : brightness` for a scalar input), and `src/things/vswitch/` — a complete Thing Model of TD + `state.json` with **no** `logic.js`.
